    ...
    case 3:
      {
        ...
        switch (_tcpStatus)
        {
          case 2:
            {
              _sendAT("AT+CSTT=\"fast.t-mobile.com\"\r\n", 5000);
              break;
            }
          case 3:
            {
          ...
          case 5:
            {
              ...
              HWSerial.print("AT+CIPSTART=\"TCP\",\"");
              HWSerial.print(MQTT_HOST);
              HWSerial.print("\",\"");
              HWSerial.print(MQTT_PORT);
...
void GSM_MQTT::_ping(void)
{
  ...
  if (pingFlag == true)
  {
    ...
    if ((currentMillis - _PingPrevMillis ) >= _KeepAliveTimeOut * 1000)
    {
      ...
      HWSerial.print(char(PINGREQ * 16));
      _sendLength(0);
    }
  }
}

void GSM_MQTT::_sendUTFString(char *c_string)
{
  int localLength = strlen(c_string);

  HWSerial.print(char(localLength / 256));
  HWSerial.print(char(localLength % 256));
  HWSerial.print(c_string);
}
...
void GSM_MQTT::_sendLength(int len)
{
  ...
  while (length_flag == false)
  {
    if ((len / 128) > 0)
    {
      HWSerial.print(char(len % 128 + 128));
      len /= 128;
    }
    else
    {
      length_flag = true;
      HWSerial.print(char(len));
    }
  }
}

void GSM_MQTT::connect(const char *UserName, const char *Password)
{
  // Serial.println("connect 1");
  connect("", true, true, UserName, Password, true, false, 0, false, "", "");
}

void GSM_MQTT::connect(const char *ClientIdentifier, bool UserNameFlag, bool PasswordFlag, 
                       const char *UserName, const char *Password, bool CleanSession, 
                       bool WillFlag, uint8_t WillQoS, bool WillRetain, const char *WillTopic, 
                       const char *WillMessage)
{
  ConnectionAcknowledgement = NO_ACKNOWLEDGEMENT;

  uint8_t msg[127];
  uint8_t index = 0, arrayLength = 0;

  // fixed header (2 bytes) - ACK TYPE (1st), Message Length (variable headers + payload)
  msg[index++] = CONNECT << 4;
  index++;

  // variable header -----------------------------
  // Protocol name + protocol level
  index += _putLenthAndChar("MQTT", &msg[index]);    // protocol name
  msg[index++] = 4; // Level 4 protocol level (b00000100)
  msg[index++] = (UserNameFlag * User_Name_Flag_Mask) + (PasswordFlag * Password_Flag_Mask)
                 + (WillRetain * Will_Retain_Mask) + (WillQoS * Will_QoS_Mask) 
                 + (WillFlag * Will_Flag_Mask) + (CleanSession * Clean_Session_Mask);
  msg[index++] = _KeepAliveTimeOut / 256; // Keep alive (MSB)
  msg[index++] = _KeepAliveTimeOut % 256; // Keep alive (LSB)


  // Limit Client ID to 23 characters. If not, set 0.
  if (strlen(ClientIdentifier) > 0 && strlen(ClientIdentifier) < 23 ) {
    Serial.println(strlen(ClientIdentifier));
    index += _putLenthAndChar(ClientIdentifier, &msg[index]);
  } else {
  ...
  // Setting will flag 
  if(WillFlag) {
    index += _putLenthAndChar(WillTopic, &msg[index]);
    index += _putLenthAndChar(WillMessage, &msg[index]);
  }
  if(UserNameFlag) {
    index += _putLenthAndChar(UserName, &msg[index]);
    if (PasswordFlag) {
      index += _putLenthAndChar(Password, &msg[index]);
    }
  }
  ...
  for (int i=0; i < (index); i++) {
    HWSerial.write((char)msg[i]);
  }
  delay(1000);
}

uint8_t GSM_MQTT::_putLenthAndChar(const char *message, uint8_t* ptrToArray, bool bitLength) 
{
  uint8_t messageLength = strlen(message);
  if (bitLength) {
    *(ptrToArray++) = messageLength / 256;
    *(ptrToArray++) = messageLength % 256;
  }
  for (uint8_t i = 0; i < messageLength; i++, ptrToArray++) {
    *ptrToArray = message[i];
  }
  return messageLength + (bitLength*2);
}


void GSM_MQTT::publish(const char *Topic, const char *Message) 
{
  publish(0, 0, 0, 0, Topic, Message);
}

void GSM_MQTT::publish(uint8_t DUP, uint8_t Qos, uint8_t RETAIN, 
                       uint16_t MessageID, const char *Topic, const char *Message)
{
  uint8_t msg[127], index = 0;
  // Message header - fixed header
  msg[index++] = (DUP * DUP_Mask) + (Qos * QoS_Mask) 
                 + (RETAIN * RETAIN_Mask) + (PUBLISH << 4);
  index++;      // fill in variable header length later

  index += _putLenthAndChar(Topic, &msg[index]);
  if (Qos > 0) {
    msg[index++] = MessageID / 256;
    msg[index++] = MessageID % 256; 
  }

  // payload ------------------------------
  index += _putLenthAndChar(Message, &msg[index], false);
  
  // Message size (not including fixed header)
  msg[1] = index-2;
...
{
  HWSerial.print(char(PUBACK * 16));
  _sendLength(2);
  HWSerial.print(char(MessageID / 256));
  HWSerial.print(char(MessageID % 256));
}
void GSM_MQTT::publishREC(unsigned int MessageID)
{
  HWSerial.print(char(PUBREC * 16));
  _sendLength(2);
  HWSerial.print(char(MessageID / 256));
  HWSerial.print(char(MessageID % 256));
}
...
{
  HWSerial.print(char(PUBREL * 16 + DUP * DUP_Mask + 1 * QoS_Scale));
  _sendLength(2);
  HWSerial.print(char(MessageID / 256));
  HWSerial.print(char(MessageID % 256));
}
...
{
  HWSerial.print(char(PUBCOMP * 16));
  _sendLength(2);
  HWSerial.print(char(MessageID / 256));
  HWSerial.print(char(MessageID % 256));
}
...
{
  HWSerial.print(char(DISCONNECT * 16));
  _sendLength(0);
  HWSerial.write("+++");
  delay(500);
  HWSerial.write("AT+CIPSHUT\r\n");
  pingFlag = false;
}
//Messages
    ...
    else
    {
      ...
      if ((ReceivedMessageType >= CONNECT) && (ReceivedMessageType <= DISCONNECT))
      {
        ...
        if (MQTT.TCP_Flag == true)
        {
          ...
          MQTT.index = 0L;
          uint32_t a = 0;
          while ((MQTT.length-- > 0) && (HWSerial.available()))
          {
            MQTT.inputString[uint32_t(MQTT.index++)] = (char)HWSerial.read();
